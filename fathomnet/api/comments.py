...
def create(uuid: str, comment_content: dto.BoundingBoxCommentContent, auth_header: Optional[dto.AuthHeader] = None) -> dto.BoundingBoxComment:
    """Create a comment."""
    res_json = Comments.post('boundingbox/{}'.format(uuid), json=comment_content.to_dict(), auth=auth_header)
...
def update(uuid: str, comment_content: dto.BoundingBoxCommentContent, auth_header: Optional[dto.AuthHeader] = None) -> dto.BoundingBoxComment:
    """Update a comment."""
...
def find_by_bounding_box_uuid(uuid: str, auth_header: Optional[dto.AuthHeader] = None) -> List[dto.BoundingBoxComment]:
...
def flag(uuid: str, value: bool, auth_header: Optional[dto.AuthHeader] = None) -> dto.BoundingBoxComment:
    """Flag a comment."""
    res_json = Comments.post('flag/{}'.format(uuid), params={'flag': value}, auth=auth_header)
    return dto.BoundingBoxComment.from_dict(res_json)
